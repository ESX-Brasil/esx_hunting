...
local Positions = {
	['StartHunting'] = { ['hint'] = '[E] Comece a Caçar', ['x'] = -769.23773193359, ['y'] = 5595.6215820313, ['z'] = 33.48571395874 },
	...
	Citizen.CreateThread(function()
		for index, v in ipairs(Positions) do
			if index ~= 'SpawnATV' then
				...
				AddTextComponentString('Ponto de caça')
	...
	if OnGoingHuntSession then
		...
		Citizen.CreateThread(function()
			...
			for index, value in pairs(AnimalPositions) do
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Veado - Animal')
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] Abate Animal', 0.4)
